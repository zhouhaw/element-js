...
export class OpenSeaAPI {
     ...
     */
    public async postOrder1(order: OrderJSON, retries = 2): Promise<Order> {
        let json
        try {
            ...
            await delay(3000)
            return this.postOrder1(order, retries - 1)
        }
        return orderFromJSON(json)
    }

    public async postOrder(order: OrderJSON, retries = 2): Promise<any> {
        let result = await this.post(`${ORDERBOOK_PATH}/orders/post/`, order)
        return result;
    }
     ...
     */
    public async getCheck(): Promise<any> {
        const result = await this.get(
            `${ORDERBOOK_PATH}/check`, {
                limit: 1
            }
        )
        return result;
